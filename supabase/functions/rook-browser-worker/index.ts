...
async function act(a:string,b:any){
  ...
  return await withPage(id,async(c,s)=>{
    ...
    if(a==="snapshot"){const v=await ev(c,s,`JSON.stringify({url:location.href,title:document.title,text:(document.body?.innerText||'').slice(0,30000),links:[...document.querySelectorAll('a[href]')].slice(0,800).map(a=>({text:(a.innerText||a.textContent||'').trim().slice(0,240),href:a.href,context:(a.closest('div,li,article')?.innerText||a.parentElement?.innerText||'').trim().slice(0,1200)})).filter(x=>/^https?:/i.test(x.href)),images:[...document.querySelectorAll('img')].slice(0,400).map(img=>({src:img.currentSrc||img.src||'',alt:(img.alt||'').trim().slice(0,240),width:img.width||0,height:img.height||0,naturalWidth:img.naturalWidth||0,naturalHeight:img.naturalHeight||0,context:(img.closest('figure,article,section,div')?.innerText||'').trim().slice(0,600)})).filter(x=>/^https?:/i.test(x.src))})`);return{ok:true,action:a,snapshot:JSON.parse(v||"{}")}}
